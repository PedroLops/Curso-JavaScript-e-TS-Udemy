...
function goGetCandies(){
   const candies = new Promise((resolve, reject)=>{
      setTimeout(()=>{
...
//Versão async
const candyShop = async()=>{
   const candies = await goGetCandies();
   const sells = await sellCandies(candies);
   console.log(sells, ' centavos (await)');
}
candyShop();

//Versão promessas (clássico)
goGetCandies().then((resultado)=>{
   sellCandies(resultado).then((resultado2)=>{
      console.log(resultado2, 'centavos (promessa)')
   })
})
