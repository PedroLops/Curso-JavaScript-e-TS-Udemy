...
const fetch = async()=>{
   const fFast = await fetchFast();
   console.log(fFast);
   const fSlow = await fetchSlow();
   console.log(fSlow);

   console.log('Program Complete! (parte 1');
};
...
const fetchParallel = async()=>{
   const fetches = await Promise.all([fetchFast(), fetchSlow()]);
   console.log(fetches);
   console.log('Program Complete! (parte 2');
};
